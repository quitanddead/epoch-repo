-----------------------------------------
 !!! PLEASE USE THE ONLINE DOCS IF YOU CAN OR READ THE HOLE FILE TO SETUP BEC ON YOUR SERVER !!!
-----------------------------------------
Http://ibattle.org/install-and-configure/
This is just a general read-me file. 
...
--dsc switch will disable the server check. can be used when bec is not running on the server. 
      note: only the config values in the [Bec] block will work unless the ip is set to 127.0.0.1

--dvc switch will disable the version check on startup.
...
Bec is Automated Rcon Tool for Battleye on ArmA2/Oa Servers
This is a Server application only, no need for anything as a client. 
...
Create Schedules tasks, such as modts, event notification., restart server, run external scripts etc.
Signing up an reporter account will also give you the ability to use Bec's bansys
...
* Ts3query for notification and more
...
Also Visit Http://ibattle.org/install-and-configure/
...
If your running a single server you have no need for the [heartbeat] in your config file.
...
if your running multiple Bec's. DO NOT SET THE SAME NAME IN THE CONFIG FILES.
it must be unique for the current server Bec is connected to.. otherwize you'll only get problems!
...
-------------------
* ConsoleColor / ConsoleHeight / ConsoleWidth *

Play around on CMD.exe and type color XX to find wanted color, not much more to say.
Does only change color of rcon screen. not ingame text color
...
Type 2.
Locks the server based on how many players are connecting within given time. server will stay locked for given time as well.
...
Note: to use Type 2 you only add in the two extra parameters. FloodSampleTime and FloodLockTime
...
Set the path to your Ts3 xml file.
This is for Ts3 Serverquery. the ts3 server query may be set up to notify ts admins regarding kick, bans, hack.
Use the provided example file to create your own ts3.xml file.

Using the Ts3 Queryer might be handy as you dont always have admin's ingame. and not every admin like to glare at the rcon console for hours ;p
The admin's you define in your Ts3_Name.xml file dosen't even need to be ingame admins.


Valid methods:

Ts3 = ts3.xml
Ts3 = C:\some\path\ts3.xml
Ts3 = \some\path\ts3.xml
...
Foreach line the player spam on the chat when time is less than 4 secs since last line was received the spam-counter increases.
...
* Heartbeat function *
------------------------------------------------------------------------------------
Here you specify all "other" Be banfiles for your other servers.. 
Once you ban on example server 1. the ban will be auto added to server 2,3,4 so on.. depends on how many files you specify.
