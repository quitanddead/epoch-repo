...
[_crate] execVM "missions\misc\fillBoxesBuilding.sqf";

_crate setVariable ["Sarge",1,true];

_crate2 = createVehicle ["USLaunchersBox",[(_coords select 0) - 6, _coords select 1,0],[], 0, "CAN_COLLIDE"];
[_crate2] execVM "missions\misc\fillBoxesGold1.sqf";
_crate2 setVariable ["Sarge",1,true];

_aispawn = [[(_coords select 0) + 20, _coords select 1,0],80,6,6,1] execVM "missions\add_unit_server.sqf";//AI Guards
sleep 5;
_aispawn = [[(_coords select 0) + 30, _coords select 1,0],80,6,6,1] execVM "missions\add_unit_server.sqf";//AI Guards
sleep 5;
_aispawn = [[(_coords select 0) + 20, _coords select 1,0],40,4,4,1] execVM "missions\add_unit_server.sqf";//AI Guards
sleep 5;
_aispawn = [[(_coords select 0) + 30, _coords select 1,0],40,4,4,1] execVM "missions\add_unit_server.sqf";//AI Guards
