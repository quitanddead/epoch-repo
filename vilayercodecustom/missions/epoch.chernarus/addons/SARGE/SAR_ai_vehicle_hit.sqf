...
if(isPlayer _ai_veh_hitsource) then {
    ...
    if (_ai_veh_group_side == SAR_AI_friendly_side) then {
        if(SAR_HITKILL_DEBUG && !( hasInterface || isDedicated ))then{diag_log format["SAR_HITKILL_DEBUG: survivor or soldier vehicle was hit by %1",_ai_veh_hitsource];};
        if((rating _ai_veh_hitsource > -10000) && (hasInterface)) then {
