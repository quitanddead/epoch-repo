...
if (SAR_HITKILL_DEBUG && (!( hasInterface || isDedicated ))) then {
...
if(isPlayer _aikiller) then {
    ...
    if (_aikilled_group_side == SAR_AI_friendly_side) then {
        if(SAR_HITKILL_DEBUG && !( hasInterface || isDedicated ))then{diag_log format["SAR_HITKILL_DEBUG: survivor or soldier was hit by %1",_aikiller];};
        ...
        if((rating _aikiller > -10000) && (hasInterface) && side _aikiller != resistance) then {
    ...
    if (_aikilled_group_side == SAR_AI_unfriendly_side) then {
        if(SAR_HITKILL_DEBUG && !( hasInterface || isDedicated ))then{diag_log format["SAR_HITKILL_DEBUG: Adjusting humanity for bandit hit by %2 for %1",_aikiller,(SAR_band_kill_value/10)];};
        _humanity = _aikiller getVariable ["humanity",0];
